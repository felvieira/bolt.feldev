...
# Add this to entrypoint.sh before starting Wrangler
echo "Creating _worker.js for Cloudflare Workers environment"
cat > ./build/client/_worker.js << EOF
// _worker.js - Generated by entrypoint.sh for Cloudflare Workers
export default {
  async fetch(request, env, ctx) {
    ...
    // Ensure process.env exists and has the values
